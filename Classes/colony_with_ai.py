...
class Colony:
    def __init__(self, biome: Biome = Biome.FOREST):
        ...
        self.military_strength = 20.0 
        self.settlement_durability = 50.0  
        self.defending_race = "dwarf"  
    ...
    def add_resource(self, resource_type, amount):
        if resource_type in self.resources:
            ...
            print(f"📦 Agregado {amount} de {resource_type}. Total: {self.resources[resource_type]}")
        else:
            print(f"⚠️ Tipo de recurso desconocido: {resource_type}")
            self.resources[resource_type] = amount  # Opcional: crea si no existe
    ...
    def calculate_military_strength(self):
        strength = 0.0
        for dwarf in self.population.iterate():
            if dwarf.occupation == Occupation.WARRIOR:
                strength += dwarf.stats.strength 
            else:
                strength += dwarf.stats.strength * 0.5 
        self.military_strength = strength
        return strength
    
    def apply_invasion_damage(self, damage_percent, casualties):
        pop = self.count_population()
        dead = min(casualties, pop // 2)  # Max 50% muere
        self.general_morale -= damage_percent * 0.5
        # Simular muertes (remover random dwarves)
        dwarves_list = list(self.population.iterate())
        random.shuffle(dwarves_list)
        for _ in range(dead):
            if dwarves_list:
                self.population.remove(dwarves_list.pop())
        lost_resources = random.choice(["wood", "stone", "meat"])  
        loss = int(damage_percent * 2)
        self.resources[lost_resources] = max(0, self.resources.get(lost_resources, 0) - loss)
        print(f"💀 Invasión causó {damage_percent}% daño! {dead} muertos. Perdidos {loss} {lost_resources}.")
    
    def apply_disaster_damage(self, event):
        damage = event["damage"]
        casualties = event["casualties"]
        self.general_morale -= damage * 0.3
        self.settlement_durability = max(0, self.settlement_durability - event.get("structural_damage", 0))
        lost_res = random.choice(["wood", "water", "meat"])
        loss = int(damage * 1.5)
        self.resources[lost_res] = max(0, self.resources.get(lost_res, 0) - loss)
        print(f"🌪️ {event['description']} en {event['biome']}! {casualties} afectados. Perdidos {loss} {lost_res}.")
    ...
    def apply_completed_tasks(self):
        ...
        for task in completed.iterate():
            ...
            if t_type == TaskType.COLLECT_WATER:
                ...
                building_name = task.metadata.get("building", "House")
    ...
    def analyze_trends(self):
        ...
        for key in self.resources.keys():
            diff = (latest.get(key) or 0) - (previous.get(key) or 0)
    ...
    def predict_shortage(self, resource_name, turns_ahead=5):
        ...
        if len(states) >= 2:
            last = states[-1].get(resource_name) or 0
            prev = states[-2].get(resource_name) or 0
            
            consumption = max(0, prev - last)
            estimated = last - (consumption * turns_ahead)
            return estimated < 10
    ...
    def get_colony_summary(self):
        """Retorna un dict con resumen de la colonia para UI/inventario."""
        summary = {
            "resources": {},  # Dict de recursos con valores
            "buildings": self.buildings.copy(),  # Copia de edificios (incluye "House": count)
            "population": [],  # Lista de dicts con dwarf info
            "general_morale": f"{self.general_morale:.1f}%",
            "biome": f"{self.biome.value.capitalize()} (Temp: {self.biome_system.temperature}°C, Danger: {self.biome_system.danger_level:.0f}%)",
            "trends": {},  # Trends de recursos
            "shortages": [],  # Lista de recursos con escasez predicha
            "houses_details": []  # Placeholder para houses con inhabitants (llénalo en game.py)
        }
        
        # Recursos con emojis y trends
        emoji_map = {"water": "💧", "wood": "🪵", "meat": "🥩", "fish": "🐟", "ore": "⚒️", "stone": "🪨", "bronze": "🥉", "iron": "🔩", "gold": "💰", "diamond": "💎"}
        trends = self.analyze_trends()
        for res, amt in self.resources.items():
            trend_diff = trends.get(res) if trends else 0
            trend_diff = trend_diff or 0  # If get returns None
            trend_emoji = "📈" if trend_diff > 0 else "📉" if trend_diff < 0 else "➡️"
            summary["resources"][res] = f"{emoji_map.get(res, '📦')} {res.capitalize()}: {amt} {trend_emoji}"
            if self.predict_shortage(res):
                summary["shortages"].append(f"{res.capitalize()} (escasez predicha en 3 días)")
        
        # Trends como dict
        if trends:
            for key in self.resources.keys():
                diff = trends.get(key) or 0
                summary["trends"][key] = diff
        
        # Población: Itera population para basic info
        for dwarf in self.population.iterate():
            happiness = dwarf.needs_system.get_overall_happiness() if hasattr(dwarf, 'needs_system') else "???"
            status = "Working" if dwarf.is_occupied else "Available"  # Use is_occupied
            summary["population"].append({
                "name": dwarf.name,
                "occupation": dwarf.occupation.value,
                "happiness": f"{happiness:.0f}%" if isinstance(happiness, (int, float)) else happiness,
                "status": status
            })
        ...
        for dwarf in self.population.iterate():
            if hasattr(dwarf, 'needs_system'):
                h = dwarf.needs_system.get_overall_happiness()
                if h > 60:
                    happy_count += 1
                elif h < 40:
                    unhappy_count += 1
        summary["morale_breakdown"] = {"happy": happy_count, "unhappy": unhappy_count}
